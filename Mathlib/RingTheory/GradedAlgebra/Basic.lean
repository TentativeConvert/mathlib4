...
theorem RingConGen.Rel.isHomogeneous_of_isHomogeneous (hr : Rel.IsHomogeneous 𝒜 r) :
...
theorem RingConGen.Rel.isHomogeneous_of_isPureHomogeneous (hr : Rel.IsPureHomogeneous 𝒜 r) :
    Rel.IsHomogeneous 𝒜 (RingConGen.Rel r) := by
  let r' : A → A → Prop := fun a b ↦ r a b ∨ (a = 0 ∧ b = 0)
  have : RingConGen.Rel r = RingConGen.Rel r' := by
    ext
    constructor
    · intro h
      induction h <;> grind [Rel.of, Rel.refl, Rel.symm, Rel.trans, Rel.add, Rel.mul]
    · intro h
      induction h with
      | of x y hxy => rcases hxy <;> grind [Rel.of, Rel.refl]
      | _ => grind [Rel.refl, Rel.symm, Rel.trans, Rel.add, Rel.mul]
  rw [this]
  apply RingConGen.Rel.isHomogeneous_of_isHomogeneous
  refine Rel.IsPureHomogeneous.isHomogeneous _ _ ?_ ?_
